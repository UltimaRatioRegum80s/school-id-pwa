...
import { QuickBehaviorFab } from "@/components/QuickBehaviorSheet";
...
export default function DashboardPage() {
  ...
  const hasExceptions = d.kpis.absent > 0;
  const exceptionsSection = (
    <div
      ...
      data-testid="panel-exceptions"
    >
      <div className="flex items-center gap-2 mb-2">
        <AlertCircle
          className={`w-4 h-4 ${hasExceptions ? "text-amber-500" : "text-muted-foreground"}`}
        />
      ...
      {hasExceptions ? (
        <button
          ...
          className="w-full flex items-center justify-between gap-3 rounded-lg px-3 py-2.5 bg-card border border-border hover:border-amber-500/50 transition-colors text-left"
          data-testid="exception-absent"
        >
          <div className="flex items-center gap-2.5">
            <span className="w-2 h-2 rounded-full bg-amber-500 flex-shrink-0" />
            <div>
              <p className="text-sm font-medium text-foreground">
                {d.kpis.absent} student{d.kpis.absent === 1 ? "" : "s"} not on campus
              </p>
              <p className="text-xs text-muted-foreground">Tap to view and follow up</p>
            </div>
          </div>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background pb-20 md:pb-6">
      ...
      <div className="mx-auto w-full max-w-lg md:max-w-none px-4 md:px-6 py-4 md:py-5 space-y-4 md:space-y-5 flex-1">
        {filterSection}
        {/* 1. The job-to-be-done: who needs attention right now */}
        {exceptionsSection}
        {/* 2. Headline numbers */}
        {kpiSection}
        {studentSummarySection}
        {/* 3. Charts and history */}
        {!isEmpty && (
